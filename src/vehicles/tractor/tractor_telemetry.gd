...
## Tractor telemetry (plan §3 ISOBUS flavor, §4.4). Adds the four ISOBUS "out" fields on
...
## and the dashboard's t.get(name) reads work unchanged (plan §2 rule 4).
##
## hitch_pos_actual / pto_state / pto_rpm are read straight out of the tractor sim
## (plan §2 rule 3 — real values); engine_load is a modeled honest value (same latitude
...
## Modeled engine load (honest, plan §2 rule 3 latitude for aux systems): throttle
